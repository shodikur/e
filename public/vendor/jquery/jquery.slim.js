...
setDocument = Sizzle.setDocument = function( node ) {
	...
	// Class
	Expr.find[ "CLASS" ] = support.getElementsByClassName && function( classNameName, context ) {
		if ( typeof context.getElementsByClassName !== "undefined" && documentIsHTML ) {
			return context.getElementsByClassName( classNameName );
		}
	};
...
jQuery.fn.extend( {
		...
		return this.each( function() {
			...
			if ( isValidValue ) {
				...
				while ( ( classNameName = classNameNames[ i++ ] ) ) {
					...
					if ( self.hasClass( classNameName ) ) {
						self.removeClass( classNameName );
					} else {
						self.addClass( classNameName );
					}
				}

			// Toggle whole className name
			} else if ( value === undefined || type === "boolean" ) {
				classNameName = getClass( this );
				if ( classNameName ) {
	...
	hasClass: function( selector ) {
		...
		while ( ( elem = this[ i++ ] ) ) {
			if ( elem.nodeType === 1 &&
				( " " + stripAndCollapse( getClass( elem ) ) + " " ).indexOf( classNameName ) > -1 ) {
					return true;
			}
		}
